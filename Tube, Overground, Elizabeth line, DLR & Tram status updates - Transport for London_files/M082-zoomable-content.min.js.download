(function(o) {
    ...
    function ZoomableContent($element, settings) {
        ...
        if (this.ceDiffx < 0 || this.ceDiffy < 0) {
            var oldWidth = this.elementWidth, oldHeight = this.elementHeight;
    ...
    ZoomableContent.prototype.setupControls = function() {
        var t = this;
        t.$zoomInButton = $('<button  title="Zoom in" class="map-button zoom-in" tabindex="0"><span class="icon plus-icon-blue"><span class="visually-hidden">Zoom in</span></span></button>');
        t.$zoomOutButton = $('<button  title="Zoom out" class="map-button zoom-out" tabindex="0"><span class="icon minus-icon-blue"><span class="visually-hidden">Zoom out</span></span></button>');
        t.$elementContainer = t.$element.wrap("<div class='panzoom-wrapper' />").parent();
        ...
        t.$zoomInButton.on("click dblclick", function(e) {
            e.stopPropagation();
            zoomClick(false);
        }).on("keydown", function(e) {
            e.stopPropagation();
        });
        t.$zoomOutButton.on("click dblclick", function(e) {
            e.stopPropagation();
            zoomClick(true);
        }).on("keydown", function(e) {
            e.stopPropagation();
    ...
    ZoomableContent.prototype.calculateDimensions = function() {
        ...
        t.minScale = Math.min(t.cHeight / t.elementHeight, t.cWidth / t.elementWidth);
        if (t.settings.isSVG) {
        ...
        t.$element.attr("data-margin-width", (t.cWidth - t.$element.width() * t.currentScale) / 2);
        t.$element.attr("data-margin-height", (t.cHeight - t.$element.height() * t.currentScale) / 2);
    };
    ...
    ZoomableContent.prototype.setupPanZoom = function() {
        ...
        $(window).resize($.proxy(t.resize, t));
    ...
    o.init = function(el, options) {
        var element = el, marginTop = 0, marginLeft = 0;
        if (typeof el === "string") {
            element = $(el);
        }
        var init = function() {
        ...
        if (element[0].tagName.toLowerCase() === "img") {
            ...
            marginTop = parseInt(element.attr("data-margin-height")) * -1;
            marginLeft = parseInt(element.attr("data-margin-width")) * -1;
            element.css("margin-top", marginTop);
            element.css("margin-left", marginLeft);
        } else {
            var zoomableContent = init();
            marginTop = parseInt(element.attr("data-margin-height"));
            marginLeft = parseInt(element.attr("data-margin-width")) * -1;
            if (!options.setPositiveMarginTop) {
